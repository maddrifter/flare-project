...
class FlareDetailViewController: UIViewController, CLLocationManagerDelegate {
    ...
    func retrieveTimeDurationFromFirebase(completion: @escaping (_ result: Int) -> ())  {
        self.ref.child("flareConstants").observeSingleEvent(of: .value, with: { (snapshot) in
            // Get user value
    ...
    func findIfBoosted(flareId: String, completion: @escaping (_ result: Bool) -> ())  {
        ...
        self.ref.child("flares/\(flareId)/boosts").observeSingleEvent(of: .value, with: { (snapshot) in
            // Get user value
            let boosts = snapshot.value as? NSDictionary
            let ifBoosted = boosts?[uid!]
    ...
    func retrieveFlareImage() {
        ...
        let storageRef = storage.reference(forURL: "gs://flame-b736e.appspot.com")
    ...
    func sendReportToDatabase(focus: String) {
        let reportRef = self.ref.child("reports")
    ...
    func blockUser() {
        let userRef = self.ref.child("users/\((FIRAuth.auth()?.currentUser?.uid)!)/blockedUsers")
    ...
    @IBAction func reportFlareAction(_ sender: AnyObject) {
        let reportActionSheet = UIAlertController(title: nil, message: nil, preferredStyle: UIAlertControllerStyle.actionSheet)
